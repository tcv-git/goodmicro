...
do
  if [ -x "$winprog" ]
  then
    exec "$winprog" -c SWD FREQ=1000 -ME -Rst
  fi
done
...
do
  ...
  then
    LD_LIBRARY_PATH="$d/lib${LD_LIBRARY_PATH:+:}${LD_LIBRARY_PATH-}" exec "$d/bin/STM32_Programmer_CLI" --connect port=SWD freq=1000 reset=SWrst mode=HOTPLUG --erase all -rst
  fi
done
...
exec openocd -f "$this_script_dir/feather_f405rg.cfg" -c "init; reset halt; stm32f4x mass_erase 0; exit"
