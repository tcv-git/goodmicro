...
do
  if [ -x "$winprog" ]
  then
    exec "$winprog" -c SWD FREQ=1000 -P "$hex" -V -Rst
  fi
done
...
do
  ...
  then
    LD_LIBRARY_PATH="$d/lib${LD_LIBRARY_PATH:+:}${LD_LIBRARY_PATH-}" exec "$d/bin/STM32_Programmer_CLI" --connect port=SWD freq=1000 reset=SWrst mode=NORMAL --write "$hex" --verify -rst -run
  fi
done
...
exec openocd -f "$this_script_dir/feather_f405rg.cfg" -c "program \"$hex\" verify reset exit"
