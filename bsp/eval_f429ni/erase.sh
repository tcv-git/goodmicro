...
do
  if [ -x "$winprog" ]
  then
    exec "$winprog" -c JTAG FREQ=1000 UR -ME -HardRst
  fi
done
...
do
  ...
  then
    LD_LIBRARY_PATH="$d/lib${LD_LIBRARY_PATH:+:}${LD_LIBRARY_PATH-}" exec "$d/bin/STM32_Programmer_CLI" --connect port=JTAG freq=1000 reset=HWrst mode=UR --erase all -hardRst
  fi
done
...
exec openocd -f "$this_script_dir/stm32429i_eval_stlink.cfg" -c "init; reset halt; stm32f4x mass_erase 0; exit"
