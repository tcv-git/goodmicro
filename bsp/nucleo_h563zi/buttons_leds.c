...
void leds_init(void)
{
    peripheral_enable(&RCC->AHB2ENR, (RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOFEN | RCC_AHB2ENR_GPIOGEN));

    GPIO_output_push_pull_slow(GPIOB, PIN0);
    GPIO_output_push_pull_slow(GPIOF, PIN4);
    GPIO_output_push_pull_slow(GPIOG, PIN4);
}

void led_red(bool on)
{
  GPIO_set_reset(GPIOG, (on ? PIN4_HI : PIN4_LO));
}

void led_amber(bool on)
{
  GPIO_set_reset(GPIOF, (on ? PIN4_HI : PIN4_LO));
}
